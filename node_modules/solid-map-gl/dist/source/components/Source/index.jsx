import { onCleanup, createEffect, createContext, useContext, createUniqueId, } from 'solid-js';
...
export const Source = props => {
    ...
    const lookup = url => {
        ...
        const source = s
            ...
            : props.source;
        source.tiles &&
            (source.tiles = ['a', 'b', 'c'].map(i => source.tiles[0].replace('{s}', i)));
        return source;
    };
    // Add Source
    ...
    switch (props.source.type) {
        case 'geojson':
            createEffect(() => {
                ...
                if (!ctx.map.isSourceLoaded(props.id))
                    return;
                source.setData(data || {});
    ...
    onCleanup(() => {
        ctx.map
            ?.getStyle()
            .layers.forEach(layer => layer.source === props.id && ctx.map.removeLayer(layer.id));
        ctx.map?.getSource(props.id) && ctx.map?.removeSource(props.id);
        debug('Remove Source:', props.id);
    });
    return (<SourceContext.Provider value={props.id}>
      {props.children}
